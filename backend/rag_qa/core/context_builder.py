...
- 核心 skill (CORE_SKILLS) 只允许从基线目录加载,后续目录的同名 skill 被拒绝
  这是为了防止第三方 skill 篡改 RAG 主干行为(答题、rerank、分类、策略)
...
class Skill:
    ...
    description: str
    inputs: list
    template: str               # markdown body,含 {var} 占位符
    include_identity: bool      # 是否在 messages 头部注入 identity
    source: str = ""            # 来源文件绝对路径(调试/审计用)
    meta: dict = field(default_factory=dict)  # 其他 frontmatter 字段(扩展位)
...
def parse_frontmatter(text: str) -> tuple:
    """解析顶部 --- ... --- 包围的 frontmatter,返回 (meta_dict, body)。

    无 frontmatter 时返回 ({}, text)。
    支持:
        key: value                                  # 标量(去引号,自动 bool)
        key:                                        # 列表
          - item1
          - item2
    """
...
class ContextBuilder:
    """加载 identity.md 与 skills/*.md(或 skills/*/SKILL.md),按 skill 名构建 OpenAI messages"""
    ...
    def __init__(
        ...
        # 规范化:单个 str/Path 或 列表 → list[Path]
        ...
        # 后续目录如果也有 identity.md,提示一下(不替换主身份)
        ...
        # skills 按目录顺序加载,后者可覆盖前者(核心 skill 除外)
        self.skills: dict = {}
    ...
    def _load_skills_from(self, prompts_dir: Path, is_baseline: bool):
        """扫描一个 prompts 目录的 skills/,递归加载所有 skill 并应用冲突策略。

        支持的布局(可混用):
        - 扁平:        skills/<name>.md
        - 嵌套:        skills/<name>/SKILL.md  (或 skill.md,Claude Code 风格)
        - 分组(任意层): skills/<group>/<name>/SKILL.md   ← 分组目录里没 SKILL.md,递归进入
                       skills/<group>/<subgroup>/<name>/SKILL.md
        skill 名按以下优先级取:
            1. frontmatter 的 name 字段(显式)
            2. 直接父目录名(嵌套)/ 文件名 stem(扁平)
        分组目录名只用于物理组织,不进入 skill name(命名空间仍是扁平)。
        """
    ...
    def _scan_skills(self, root: Path):
        """递归扫描 root,产出 (skill_file, fallback_name) 元组。

        规则:
        - 当前目录下的 *.md(非 SKILL.md/skill.md)→ 扁平 skill,fallback = 文件 stem
        - 子目录里有 SKILL.md/skill.md → 整个子目录是一个 skill,fallback = 子目录名
        - 子目录里没 SKILL.md → 视为分组目录,递归进入
        """
        # 直接子文件(扁平 skill)
        for f in sorted(root.glob("*.md")):
            if f.name.lower() == "skill.md":
                continue  # SKILL.md/skill.md 留给嵌套语义,不在 root 顶层 emit
            yield f, f.stem
        # 直接子目录:嵌套 skill 或分组目录
    ...
    def _register(self, skill: Optional[Skill], is_baseline: bool):
        """把 skill 注册到 self.skills,应用核心保护与覆盖告警"""
        if skill is None:
            return
        ...
        if existing is not None:
            if skill.name in self.core_skills and not is_baseline:
                ...
                return
            logger.warning(
                f"skill '{skill.name}' 被覆盖: {skill.source} 覆盖 {existing.source}"
            )
    ...
    def build_messages(
        ...
        **variables,
        ...
        """按 skill 名构建 OpenAI messages 列表。

        Args:
            skill: skill 名(对应 frontmatter 的 name 或文件/目录名 fallback)
            include_identity: None 则使用 skill 自身配置;显式 True/False 覆盖
            history: [{role, content}, ...] 形式的会话历史,可选
            **variables: 模板占位符的实际值
